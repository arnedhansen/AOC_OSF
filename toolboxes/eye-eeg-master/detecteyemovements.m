...
%%%%%%% BAD WORKAROUND
% Calculate the current total number of elements in badvector
totalElements = numel(badvector);

% Calculate the remainder when totalElements is divided by 1500
remainder = mod(totalElements, 1500);

% If remainder is not zero, we need to remove some elements
if remainder ~= 0
    % Calculate the new size by subtracting the remainder
    newSize = totalElements - remainder;

    % Resize badvector to the new size
    badvector = badvector(1:newSize);
end

%
badvector = reshape(badvector,1,size(EEG.data,2),[]);
